...
public class TCH_08_VerifySearchByHotel extends BaseClass {
	...
	public void myTest(Map<String, String> excelData) throws InterruptedException, IOException {
		...
		try{
			...
			int starRating = Integer.parseInt(excelData.get("StarRating"));
			...
			String amenities = excelData.get("Amenities");
			...
			resultpage.clickOnRating(starRating);
			Thread.sleep(3000);
			resultpage.validateStarRating(starRating,Log, screenShots);
			
			resultpage.clickOnInPolicy();
			resultpage.validatePolicy(Log, screenShots,policy);
			
			
			
			double[] sliderRange = resultpage.getDefaultValueInSlider(Log, screenShots);
			double  minimum = sliderRange[0];
			double maximum  = sliderRange[1];
			double minValue=minimum+4000;
			double maxValue=maximum-4000;
			Thread.sleep(2000);
			resultpage.setMaximumSliderValue(driver,maxValue);
			Thread.sleep(2000);
			resultpage.verifyPriceRangeValuesOnResultScreen(Log, screenShots);
			
			resultpage.getTotalHotelCountBeforeApplyingFilter(Log, screenShots);
			Thread.sleep(2000);
			resultpage.clickOnShowReCommended();
			Thread.sleep(2000);
			resultpage.validateRecommendedFilter(Log, screenShots);
			Thread.sleep(2000);
			resultpage.getTotalHotelCountAfterApplyingFilter(Log, screenShots);
