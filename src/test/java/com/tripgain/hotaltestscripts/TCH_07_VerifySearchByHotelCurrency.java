...
public class TCH_07_VerifySearchByHotelCurrency extends BaseClass {
	...
	public void myTest(Map<String, String> excelData) throws InterruptedException, IOException {
		...
		try{
			...
			double userRatingNumber = Double.parseDouble(excelData.get("UserRatingNumber"));
		...
		//	String SearchByHotelClearAndSearch = excelData.get("SearchByHotelClearAndSearch");
			...
			String userRating = excelData.get("UserRating");
			...
			resultpage.clickOnUserRating(userRating);
			resultpage.validateUserRating(userRatingNumber,Log, screenShots);
			
		Thread.sleep(3000);
			
			int price[]	=resultpage.selectPerNightPrice(Log, screenShots);
			
			int min=price[0];
			int max=price[1];
			Thread.sleep(3000);
				
			resultpage.verifyPerNightPrice(min,max,Log, screenShots);
			...
			Thread.sleep(2000);
			resultpage.setMinimumSliderValue(driver,minValue);
			Thread.sleep(2000);
