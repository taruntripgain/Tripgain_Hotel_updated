...
public class TCH_05_VerifySearchFunctionalityForHotalApplyingFliters extends BaseClass {
	...
	public void myTest(Map<String, String> excelData) throws InterruptedException, IOException {
		...
		try{
			...
			String distanceDescending = excelData.get("Distance Descending");
			String distanceAscending = excelData.get("Distance Ascending");
			String priceHighToLow = excelData.get("Price : High to Low");
			String priceLowToHigh = excelData.get("Price : Low to High");
			String starDescending = excelData.get("Star : Descending");
			String starAscending = excelData.get("Star : Ascending");
			String hotelDescending = excelData.get("Hotels : Descending");
			String hotelAscending = excelData.get("Hotels : Ascending");
			...
			resultpage.clickOnSort();
			resultpage.checkOnSortOptions(distanceDescending);
			resultpage.validateDistanceInDescendingOrder(Log, screenShots);
			
			Thread.sleep(5000);
			resultpage.clickOnSort();
			resultpage.checkOnSortOptions(distanceAscending);
			resultpage.validateDistanceInAscendingOrder(Log, screenShots);
			Thread.sleep(5000);
			resultpage.clickOnSort();
			resultpage.checkOnSortOptions(priceHighToLow);
			resultpage.validatePriceInDescending(Log, screenShots);
			Thread.sleep(5000);
			resultpage.clickOnSort();
			resultpage.checkOnSortOptions(priceLowToHigh);
			resultpage.validatePriceInAscending(Log, screenShots);
			Thread.sleep(5000);
			resultpage.clickOnSort();
			resultpage.checkOnSortOptions(starDescending);
			resultpage.validateStarRatingInDescendingOrder(Log, screenShots);
			Thread.sleep(5000);
			resultpage.clickOnSort();
			resultpage.checkOnSortOptions(starAscending);
			resultpage.validateStarRatingInAscendingOrder(Log, screenShots);
			
			Thread.sleep(5000);
			resultpage.clickOnSort();
			resultpage.checkOnSortOptions(hotelDescending);
			resultpage.validateHotelSortingDescending(Log, screenShots);
			Thread.sleep(5000);
			
			resultpage.clickOnSort();
			resultpage.checkOnSortOptions(hotelAscending);
			resultpage.validateHotelSortingAscending(Log, screenShots);
