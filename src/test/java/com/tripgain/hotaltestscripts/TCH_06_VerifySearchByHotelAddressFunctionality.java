...
public class TCH_06_VerifySearchByHotelAddressFunctionality extends BaseClass {
	...
	public void myTest(Map<String, String> excelData) throws InterruptedException, IOException {
		...
		try{
			...
			/*
			String otp = OtpReader.getOTPFromGmail();
...
*/
			...
			int selectHotelBasedOnIndex = Integer.parseInt(excelData.get("selectHotelBasedOnIndex"));
			int selectHotelInDetailsPageBasedOnIndex = Integer.parseInt(excelData.get("selectHotelInDetailsPageBasedOnIndex"));
			
			String SearchByHotelClearAndSearch = excelData.get("SearchByHotelClearAndSearch");
			String currency = excelData.get("Currency");
			...
			//Search by hotels
            resultpage.clickOnSearchByHotel();
			
			String hotelName=resultpage.getHotelName();
			
			resultpage.SearchByHotel(hotelName);
			
			resultpage.validateHotelName(hotelName,Log, screenShots);
			
			Thread.sleep(3000);
			//search by locations
			resultpage.SearchByHotelClearAndSearch(SearchByHotelClearAndSearch);
			resultpage.validateHotelAddress(SearchByHotelClearAndSearch,Log, screenShots);
			
			String amenitiesName =resultpage.getAmenitiesRandomly();
			System.out.println(amenitiesName);
			resultpage.clickOnAmenities(amenitiesName);
			
			//Validate Currency
			resultpage.defaultCurrencyValue(Log, screenShots);
			resultpage.clickOnCurrencyDropDown();
			System.out.println(currency);
			resultpage.selectCurrencyDropDownValues(driver,currency);
			resultpage.validateCurrency(currency,Log,screenShots);
			
			resultpage.selectHotelBasedOnIndex(selectHotelBasedOnIndex);
			
			Thread.sleep(3000);
			
			tripgain_Hotel_hotelDetailPage.validateCurrencyInHotelDetailsPage(currency,Log,screenShots);
			
			tripgain_Hotel_hotelDetailPage.validateAmenities(amenitiesName,Log,screenShots);
			
			Thread.sleep(3000);
			
			tripgain_Hotel_hotelDetailPage.selectHotelInDetailsPageBasedOnIndex(selectHotelInDetailsPageBasedOnIndex);
			
			tripgain_Hotel_BookingPage.currencyInBookingPage(currency,Log,screenShots);
